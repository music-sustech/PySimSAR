...
class ChirpScalingAlgorithm(ImageFormationAlgorithm):
    ...
    def azimuth_compress(self, phase_history, radar, trajectory) -> SARImage:
        ...
        R_ref = max(R_ref_bin * range_bin_spacing, 1.0)
    ...
    def _apply_residual_rcmc(
        ...
        for k in range(n_az):
            ...
            # range bin b is: (b * rbs - R_ref) * a_k / rbs bins
            # = (b - R_ref/rbs) * a_k = (b - R_ref_bin) * a_k
            # Total source position: b + residual = b + (b - R_ref_bin)*a_k
            # = b * (1 + a_k) - R_ref_bin * a_k
            # = b / D_k - R_ref_bin * a_k
            R_ref_bin = R_ref / range_bin_spacing
            src_positions = range_bins / D_k - R_ref_bin * a_k
