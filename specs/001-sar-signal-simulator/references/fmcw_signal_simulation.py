...
def simulate_fmcw_radar():
    ...
    plt.title("Simulated LO Phase Noise $\phi_{pn}(t)$")
...
""" Code Architecture BreakdownContinuous Kinematics Check: 
 * Notice how tau is calculated as an array based on the t array. By plugging t_delayed directly into the $t^2$ portion of the received phase, the script inherently captures the Range-Doppler coupling effect we discussed.
 * Precision Handling: We model the entire complex exponent np.exp(1j * ...) directly. Because Python uses 64-bit floats by default, it retains enough precision to handle numbers as large as $77$ GHz multiplied by microseconds without devastating truncation errors in the phase.
 * Phase Noise Emulation: I generated $1/f$ (pink) noise. This is much more realistic for oscillators than white noise. We then use scipy.interpolate.interp1d to figure out exactly what the delayed phase noise $\phi_{pn}(t - \tau(t))$ looks like, managing the boundary conditions where $t < \tau$.
 * Verification: The code prints the theoretical range and Doppler beat frequencies so you can visually verify that the FFT peak lands exactly where the continuous-time math says it should. """